// ============ 1. قاعدة البيانات والبيانات الافتراضية (Auto-Seed) ============
let db = JSON.parse(localStorage.getItem('schoolDB_PRO_ERP'));

let todayISO = new Date(new Date().getTime() - (new Date().getTimezoneOffset() * 60000)).toISOString().split('T')[0];
...
if (!db) {
    db = {
        ...
        staff: [
            { id: 1, name: 'ياسر محمود', role: 'مدرس لغة عربية', salary: 600000, payments: [] }
        ],
        expenses: [
            { id: 1, desc: 'صيانة وتصليح كهرباء', amount: 50000, date: todayISO }
        ]
    };
    localStorage.setItem('schoolDB_PRO_ERP', JSON.stringify(db));
}
...
function saveDB() { localStorage.setItem('schoolDB_PRO_ERP', JSON.stringify(db)); }
function saveSetup() {
...
function initApp() {
    if(!db.staff) db.staff = []; if(!db.expenses) db.expenses = [];
    ...
    // ضبط تواريخ الإدخال الافتراضية إلى تاريخ اليوم
...
function updateFinance(s) {
    ...
    document.getElementById('payments-history').innerHTML=s.payments.map((p,i)=>`<div class="list-item flex-between p-2"><span>${p.amount.toLocaleString()} د.ع | ${p.date}</span> <button class="btn-3d danger btn-small m-0" onclick="delPayment(${i})"><i class="fas fa-trash"></i></button></div>`).join('');
}
...
function payStaff(id) { customPrompt("المبلغ المراد صرفه للموظف (د.ع):", amt=>{ let v=parseFloat(amt); if(v>0){ db.staff.find(x=>x.id===id).payments.push({amount:v, date:new Date().toISOString().split('T')[0]}); saveDB(); renderStaff(); renderDual(); renderDaily(); if(typeof Swal !== 'undefined') Swal.fire({toast:true, position:'top-end', icon:'success', title:'تم صرف المبلغ بنجاح', showConfirmButton:false, timer:1500}); } }); }
function renderStaff() { document.getElementById('staff-list').innerHTML = db.staff.map(st => { let paid = st.payments.reduce((s,p)=>s+p.amount,0); return `<div class="list-item flex-between"><div><strong><i class="fas fa-user-tie text-primary"></i> ${st.name}</strong> <small>(${st.role})</small><br><small>الراتب: ${st.salary.toLocaleString()} | المصروف: <span style="color:#2ecc71">${paid.toLocaleString()}</span> | الباقي: <span style="color:#e74c3c">${(st.salary-paid).toLocaleString()}</span></small></div><div class="flex-row"><button class="btn-3d success btn-small m-0" onclick="payStaff(${st.id})"><i class="fas fa-hand-holding-usd"></i> صرف</button> <button class="btn-3d warning btn-small m-0" onclick="editStaff(${st.id})"><i class="fas fa-pen"></i></button> <button class="btn-3d danger btn-small m-0" onclick="deleteStaff(${st.id})"><i class="fas fa-trash"></i></button></div></div>`; }).join('') || '<div class="text-center mt-3">لا توجد بيانات</div>'; }

// ============ 8. المصروفات التشغيلية ============
function openAddExpenseModal() { document.getElementById('exp-desc').value=''; document.getElementById('exp-amount').value=''; document.getElementById('exp-date').value = new Date().toISOString().split('T')[0]; showModal('add-expense-modal'); }
...
function renderDual() {
    ...
    let net = col - sal - exs; let netEl = document.getElementById('dual-net'); netEl.innerText = net.toLocaleString(); netEl.style.color = net>=0 ? '#2ecc71' : '#e74c3c';
}
